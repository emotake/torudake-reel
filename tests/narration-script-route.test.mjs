...
test("rejects temporarily unavailable pop-character aliases without calling OpenAI", async () => {
  setNarrationScriptTestEnvironment();
  const originalFetch = globalThis.fetch;
...
test("uses the shared character-v1 persona for the active comedy style", async () => {
  ...
  try {
    ...
    assert.match(prompts[0], /声の雰囲気: ハイテンショントーク/);
    assert.match(prompts[0], /短い導入からすぐ本題/);
    assert.match(prompts[0], /要点の直前には意味のある短い一拍/);
    assert.match(prompts[0], /結論だけを明瞭に強調/);
    assert.match(prompts[0], /台本の文字数: 104〜125字/);
