  ...
  return (
    <>
      ...
      <article>
        <h2>AI処理の利用上限</h2>
        <p>
          AI処理には、文字起こし、高精度再解析、AI台本の生成、AI音声の生成が含まれます。初回ナレーションは台本が正常に生成された時点で1回分を使用し、続く初回音声と内部の自動調整では追加回数を使用しません。作成後の再生成などは正常に完了するごとに1回分を使用します。正常に完了したAI処理の回数は、動画を保存せず編集を終了した場合も戻りません。
        </p>
        <dl className="legalDetails">
          <div>
            <dt>無料体験</dt>
