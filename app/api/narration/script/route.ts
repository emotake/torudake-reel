...
const STYLE_INSTRUCTIONS: Record<NarrationStyle, string> = {
  ...
  comedy:
    "テンポのよいバラエティトーク。真剣な状況説明→一拍→短く勢いのある返し、または予想外のオチで緩急を作る。状況説明は標準語を基本とし、短い返しとオチだけ自然で控えめな関西イントネーションを使う",
};
...
export async function POST(request: Request) {
  ...
  const characterRules =
    style === "comedy"
      ? `
- 「テンポコメディ」では、冒頭3秒以内に最初の意外性を置き、「真剣な実況→一拍→短く勢いのある返し／オチ」の落差を必ず作ってください。
- 関西イントネーションは短い返しにだけ控えめに使い、特定の実在人物の声質、口癖、笑い方、決め台詞、話速、間合いは模倣しないでください。
