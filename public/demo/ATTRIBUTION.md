...
The four `public/demo/voices/*-v5.wav` previews replace v4 after users reported a misread bright-male sample and audible background noise. They use the production `gpt-realtime-2.1-mini` model and the earlier `2026-08-13-quality-v4` profile, so they demonstrate each base voice rather than exact parity with the current Japanese-tuned instructions. A temporary authenticated endpoint accepted only the four repository-defined scripts, required realtime generation without fallback, and transcribed every raw take for selection; it was removed before the final build. Twenty initial takes plus five bounded replacement takes for the bright voice were compared. The selected take for each voice preserved the opening, important words, and ending after Japanese orthographic normalization. Mastering applies a 70 Hz high-pass, 10.5 kHz low-pass, gentle noise reduction, approximately -20.8 LUFS normalization, a -3 dBTP ceiling, and an exact 350 ms post-speech duration. Full provenance, transcripts, hashes, and measurements are recorded in `public/demo/voices/manifest-v5.json`.

The active fixed-preview paths and readiness metadata are defined in `lib/voice-sample-catalog.ts`. On 2026-08-23, the user selected screening choice B for both character styles: OpenAI's built-in `shimmer` voice for `party` (ポップキャラクター) and `verse` for `comedy` (ハイテンショントーク). The selected source samples are `CV-3EA1DE9DE6` and `CV-444DFF3BE1` from the fixed 24-sample screening plan. Both source transcripts matched the fixed script after NFKC, punctuation, and whitespace normalization; both had a zero clipped-sample ratio. No audio was regenerated for this promotion. The existing `scripts/master-voice-samples.mjs --version v6 --styles comedy,party` command processed only those two approved raw takes locally, using the same filtering, loudness, and post-roll policy as v5. The resulting source hashes, generation-record hash, and measurements remain recorded in `public/demo/voices/manifest-v6.json` as historical provenance. The `party` preview and production option were removed from the public catalog on 2026-08-23 while its Japanese intonation is being revised; the public preview catalog currently contains `calm`, `bright`, and `comedy` only.
...
## Home photo-mode stills

`public/demo/torudake-photo-{flowers,brunch,dog}-v1.jpg` were generated on 2026-08-15 with OpenAI's built-in image generation tool specifically for the home-page photo-reel entry. They depict fictional everyday-life subjects, contain no identifiable person, logo, or copied text, and do not derive from third-party stock media. The selected source was cropped into three distinct 600 × 400 JPEGs and compressed locally for the compact mode-card presentation.
